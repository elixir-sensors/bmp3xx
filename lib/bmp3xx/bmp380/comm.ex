defmodule BMP3XX.BMP380.Comm do
  @moduledoc false
  ...
  @reg_measurement_data 0x04
  @reg_int_ctrl 0x19
  @reg_calibration 0x31
  @reg_if_conf 0x1A
  @reg_pwr_ctrl 0x1B
  @reg_osr 0x1C
  @reg_odr 0x1D
  ...
  def reset(transport) do
    case Transport.write(transport, [@reg_cmd, <<0xB6>>]) do
      :ok ->
        Process.sleep(10)
  ...
  def set_odr_and_filter_settings(transport) do
    ...
    with :ok <- Transport.write(transport, [@reg_osr, <<0::2, osr_t::3, osr_p::3>>]),
         :ok <- Transport.write(transport, [@reg_odr, <<0::3, odr_sel::5>>]) do
      Transport.write(transport, [@reg_config, <<0::4, iir_filter::3, 0::1>>])
    end
  end
  ...
  @spec get_calibration(Transport.t()) :: {:ok, <<_::168>>} | {:error, any}
  def get_calibration(transport) do
    Transport.write_read(transport, [@reg_calibration], 21)
  end

  @spec get_output(Transport.t()) :: {:error, any} | {:ok, <<_::48>>}
